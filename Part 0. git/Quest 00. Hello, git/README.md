...
* git 외의 버전관리 시스템에는 무엇이 있나요? git은 그 시스템과 어떤 점이 다르며, 어떤 장점을 가지고 있나요?
  ...
  * 버전 관리 시스템은 로컬 버전 관리 시스템(Local VCS), 중앙집중식 버전 관리 시스템(CVCS), 분산 버전 관리 시스템(DVCS)로 구분할 수 있다. 
    * 로컬 버전 관리 시스템(Local VCS)
      pc 내에 데이터베이스를 저장하여 관리, 협업이 힘들고 바이러스나 기기 고장 시 복구 불가능. (RCS, SCCS 등)
    * 중앙집중식 버전 관리 시스템(CVCS) 
      별도의 서버를 두고 클라이언트가 중앙 서버에서 파일을 받아서 사용, 협업이 가능하고 누가 무엇을 하는지 쉽게 알 수 있는 등 관리가 용이하나 중앙 서버에 문제가 생기면 복구가 불가능(CVS, SVN 등)
