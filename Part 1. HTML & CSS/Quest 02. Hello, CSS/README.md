...
* `float` 속성은 왜 좋지 않을까요?
  ...
  * 자식 요소의 높이를 무시하여 예상치 못한 결과를 출력하는 문제를 가지고 있다. clear 속성을 사용하거나 overflow 속성을 사용하여 해결할 수 있다.

* Flexbox(Flexible box)와 CSS Grid의 차이와 장단점은 무엇일까요?

  * Flexbox는 기본적으로 단일 차원(행 또는 열)의 배치, CSS Grid는 행과 열의 두 가지 차원을 동시에 배치
  * Flexbox는 내용을 내부에 정렬하는데 유리, CSS Grid는 더 큰 그림을 만드는데 유리
  * 대체재가 아닌 보완재적 관계로 두 가지 모두 서로 다르게 사용할 수 있다.
